...
class cabinet {
  ...
  protected render = {
    overall: () => {
      ...
      const { offsetZ } = this.config.cabinet as any;
      let index = 0;
      this.dm.add(all);
      all.addChild(plane);
    ...
    group: (item: cabinet.item) => {
      ...
      const { columnOffset, x } = this.config.cabinet as any;
      let index = 0;
      let max = 0;
      this.dm.add(group);
    ...
    groupWall: (item: cabinet.item) => {
      ...
      const { x, y, z, columnOffset } = this.config.cabinet as any;
      for (let i of item.child || []) {
    ...
    taskLength: (
      ...
      const { x, y } = this.config.cabinet as any;
      const visible = item.cards || 0 > 0 ? true : false;
      node.s3([74, 112, 20]);
      node.s({
        "texture.cache": true,
        "3d.visible": visible,
    ...
    column: (item: cabinet.item, max: number) => {
      ...
      const { columnOffset, y } = this.config.cabinet as any;
      const h = y / 3;
    ...
    // 地板
    plane: () => {
      const { x, y, z, offsetZ, columnOffset } = this.config.cabinet as any;
  ...
  protected parts = {
    ...
    box: () => {
      ...
      const { x, y, z, boxColor } = this.config.cabinet as any;
      box.s3(x, y, z);
      box.s({
        "all.color": boxColor,
      });
    ...
    fillNode: (item: cabinet.item, m: number) => {
      ...
      const { x, z, y, columnOffset, boxColor } = this.config.cabinet as any;
      const p3 = {
        x,
        z,
    ...
    head: (item: cabinet.item, m: number) => {
      ...
      const { x, z, y, columnOffset, boxColor, column } = this.config
        .cabinet as any;
      const p3 = [0, 0, 0];
    ...
    tail: (item: cabinet.item, h: number) => {
      const { x, z, columnOffset, boxColor } = this.config.cabinet as any;
    ...
    brake: (item: cabinet.item) => {
      ...
      const { x, y, z } = this.config.cabinet as any;
      brake.s({
        shape3d: "brake",
      });
  ...
  protected reset = {
    ...
    base: () => {
      const dmid = document.getElementById(
        this.config.domId || ""
      ) as HTMLDivElement;
